...
- Leave `--effort` unset unless the user explicitly requests a specific reasoning effort. Copilot CLI has no per-call reasoning knob, so the companion maps effort levels to Copilot model tiers (low→`claude-opus-4.6-fast`, medium→`claude-sonnet-4.5`, high/xhigh→`claude-opus-4.6`).
- Leave model unset by default. Only add `--model` when the user explicitly asks for a specific model.
- Accepted model aliases are `fast`, `opus`, `sonnet`, `haiku`. Pass any concrete Copilot model name such as `claude-opus-4.6` or `gpt-5.2-codex` through unchanged with `--model`.
